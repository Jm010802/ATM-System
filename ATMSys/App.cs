...
namespace ATMSys
{
    public class App : IATMApp, ITransaction, IUserBankAccount
    {
        ...
        public void Initialization()
        {
            ...
            {
                new UserBankAccount() { Id=1, FullName = "Jm Nicolas", AccountNumber=333111, CardNumber = 123123, CardPin = 111111, AccountBalance = 2000.00m, IsLocked = false },
                new UserBankAccount() { Id=2, FullName = "Patrick Potian", AccountNumber=111222, CardNumber = 456456, CardPin = 222222, AccountBalance = 1500.30m, IsLocked = true },
            };
        ...
        public void PlaceDeposit()
        {
            ...
            var transaction_amt = Validator.Convert<int>($"amount {Screen.currency}");
        ...
        public void PerformThirdPartyTransfer(ThirdPartyTransfer ThirdPartyTransfer)
        {
            if (ThirdPartyTransfer.TransferAmount <= 0)
            {
                Utility.PrintMessage("Amount needs to be more than zero. Try again.", false);
                return;
            }
            ...
            if (ThirdPartyTransfer.TransferAmount > selectedAccount.AccountBalance)
            {
                Utility.PrintMessage("Withdrawal failed. You do not have enough " +
                    $"fund to withdraw {Utility.FormatAmount(ThirdPartyTransfer.TransferAmount)}", false);
                return;
            }

            if (selectedAccount.AccountBalance - ThirdPartyTransfer.TransferAmount < minimum_kept_amt)
            {
            ...
            var selectedBankAccountReceiver = (from b in _accountList
                                               where b.AccountNumber == ThirdPartyTransfer.RecipientBankAccountNumber
            ...
            if (selectedBankAccountReceiver.FullName != ThirdPartyTransfer.RecipientBankAccountName)
            {
            ...
            InsertTransaction(selectedAccount.Id, TransactionType.ThirdPartyTransfer, +
                -ThirdPartyTransfer.TransferAmount, "Transfered " +
                $" to {selectedBankAccountReceiver.AccountNumber} ({selectedBankAccountReceiver.FullName})");


            selectedAccount.AccountBalance = selectedAccount.AccountBalance - ThirdPartyTransfer.TransferAmount;
            ...
            selectedBankAccountReceiver.AccountBalance = selectedBankAccountReceiver.AccountBalance + ThirdPartyTransfer.TransferAmount;

            Utility.PrintMessage("You have successfully transferred out " +
                $" {Utility.FormatAmount(ThirdPartyTransfer.TransferAmount)} to {ThirdPartyTransfer.RecipientBankAccountName}", true);
        }
        ...
        private bool PreviewBankNotesCount(decimal amount)
        {
            ...
            Utility.PrintUserInputLabel($"{Screen.currency} 100 x {hundredNotesCount} = {100 * hundredNotesCount}", true);
            Utility.PrintUserInputLabel($"{Screen.currency} 50 x {fiftyNotesCount} = {50 * fiftyNotesCount}", true);
            Utility.PrintUserInputLabel($"{Screen.currency} 10 x {tenNotesCount} = {10 * tenNotesCount}", true);
        ...
        public void ViewTransaction()
        {
            ...
            {
                var table = new ConsoleTable("Id", "Transaction Date", "Type", "Descriptions", "Amount " + Screen.currency);
